<?php
/**
 * Email Handler - Credit Note Email with PDF Attachment
 *
 * Extends WooCommerce email system to send credit notes created in Lexware.
 * Automatically triggered when credit note PDF is ready.
 *
...
class Credit_Note extends \WC_Email {
    ...
    /**
     * Credit Note number from Lexware (e.g., GS0023)
     *
     * @since 1.0.0
     * @var string
     */
    ...
    public function __construct() {
        $this->id = 'lexware_mvp_credit_note';
        $this->title = __('Lexware Gutschrift', WC_LEXWARE_MVP_TEXT_DOMAIN);
        $this->description = __('Email mit Lexware-Gutschrift als PDF-Anhang', WC_LEXWARE_MVP_TEXT_DOMAIN);

        $this->template_html = 'emails/lexware-credit-note.php';
        $this->template_plain = 'emails/plain/lexware-credit-note.php';
        $this->template_base = plugin_dir_path(dirname(dirname(__FILE__))) . 'templates/';

        $this->customer_email = true;

        // Triggers
        add_action('wc_lexware_mvp_send_credit_note_email', array($this, 'trigger'), 10, 3);
     ...
     * Trigger email
     *
     * Called via 'wc_lexware_mvp_send_credit_note_email' action when PDF is created.
     * Attaches PDF and sends email to customer.
     *
     * @since 1.0.0
     * @param int    $order_id       Order ID
     * @param int    $cn_record_id   Credit note database record ID
     * @param string $pdf_filename   PDF filename to attach
     */
    public function trigger($order_id, $cn_record_id, $pdf_filename) {
        $this->setup_locale();
        ...
        // Load credit note number and ID from order meta
        $this->credit_note_number = $order->get_meta('_lexware_credit_note_number', true);
        $this->credit_note_id = $order->get_meta('_lexware_credit_note_id', true);
        ...
        if (\WC_Lexware_MVP_PDF_Handler::pdf_exists($pdf_filename)) {
            ...
            if ($file_size <= $max_size) {
                ...
                \WC_Lexware_MVP_Logger::debug('Credit Note PDF als E-Mail-Anhang hinzugefügt', array(
                    'order_id' => $order_id,
                    'pdf_path' => $pdf_path,
                ...
                \WC_Lexware_MVP_Logger::warning('Credit Note PDF zu groß für E-Mail-Anhang', array(
                    'order_id' => $order_id,
                    'pdf_path' => $pdf_path,
                    'file_size' => size_format($file_size),
                    'max_size' => size_format($max_size)
                ...
                $order->add_order_note(
                    sprintf(
                        __('⚠️ Lexware Gutschrift PDF zu groß für E-Mail (%s). Maximum: %s. Bitte manuell versenden.', WC_LEXWARE_MVP_TEXT_DOMAIN),
            ...
            \WC_Lexware_MVP_Logger::error('Credit Note PDF-Datei nicht gefunden', array(
                'order_id' => $order_id,
                'pdf_path' => $pdf_path,
        ...
        if ($this->is_enabled() && $this->get_recipient()) {
            \WC_Lexware_MVP_Logger::debug('Credit Note E-Mail wird versendet', array(
                'order_id' => $order_id,
            ...
            // Track email success
            add_action('wp_mail_failed', function($error) use ($order_id) {
                \WC_Lexware_MVP_Logger::error('Credit Note wp_mail failed', array(
                    'order_id' => $order_id,
                    'error' => is_wp_error($error) ? $error->get_error_message() : 'Unknown error'
                ));
            }, 10, 1);

            \WC_Lexware_MVP_Logger::info('Sending credit note email via WC_Email->send()', array(
                'order_id' => $order_id,
                'recipient' => $this->get_recipient(),
            ...
            \WC_Lexware_MVP_Logger::info('Credit Note email sent via WC_Email->send()', array(
                'order_id' => $order_id,
                'recipient' => $this->get_recipient()
            ));
            ...
            $order->add_order_note(
                sprintf(
                    __('📧 Lexware Gutschrift per E-Mail versendet an %s', WC_LEXWARE_MVP_TEXT_DOMAIN),
                    $this->get_recipient()
                )
            );
            \WC_Lexware_MVP_Logger::info('Credit Note E-Mail erfolgreich versendet', array(
                'order_id' => $order_id,
                'recipient' => $this->get_recipient()
            ));
        }
    ...
    public function get_default_subject() {
        return __('Ihre Gutschrift {credit_note_number} für Bestellung #{order_number}', WC_LEXWARE_MVP_TEXT_DOMAIN);
    }
    ...
    public function get_default_heading() {
        return __('Ihre Gutschrift', WC_LEXWARE_MVP_TEXT_DOMAIN);
    }
